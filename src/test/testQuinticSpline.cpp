...
int main()
{
  ...
  cubicSpline.update(x, y);
  int n = x.size()-1;

  for (int i = 0; i < n; ++i)
  {
    Marker mk = Marker(i, 0.0);
    h.push_back(cubicSpline.getHeadingRad(mk));
    k.push_back(cubicSpline.getCurvature(mk));
  }
  {
    Marker mk = Marker(n-1, 1.0);
    h.push_back(cubicSpline.getHeadingRad(mk));
    k.push_back(cubicSpline.getCurvature(mk));
  }

  QuinticSpline spline;
  spline.update(x, y, h, k);
  ...
  for (int i = 0; i < n; ++i)
  {
    ...
    allTestsPassed &= checkDoubles("Heading do not match", h1, h[i], i);
    allTestsPassed &= checkDoubles("Heading do not match", h2, h[i+1], i);

    double k1 = spline.getCurvature(mk1);
    double k2 = spline.getCurvature(mk2);

    allTestsPassed &= checkDoubles("Curvature do not match", k1, k[i], i);
    allTestsPassed &= checkDoubles("Curvature do not match", k2, k[i+1], i);
  }
